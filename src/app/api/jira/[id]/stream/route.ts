...
import { jiraTasks, jiraTaskEvents } from "@/lib/db/schema";
...
export async function GET(
  ...
  if (!task) {
    return new Response(JSON.stringify({ error: "Jira task not found" }), {
      status: 404,
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const existingEvents = db
        .select()
        .from(jiraTaskEvents)
        .where(eq(jiraTaskEvents.jira_task_id, id))
        .orderBy(asc(jiraTaskEvents.created_at))
        .all();
